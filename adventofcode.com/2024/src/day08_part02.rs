    ...
    let results = result
        ...
        .flat_map(|chunk| {
            ...
            .flat_map(|sats| {
                ...
                let left_results: Vec<_> = successors(
                    Some(sats[0].0),
                    |pos| {
                ...
                let right_results: Vec<_> = successors(
                    Some(sats[1].0),
                    |pos| {
                        let new_pos = pos - diff;
                        if x_bound.contains(&new_pos.x) && y_bound.contains(&new_pos.y) {
...
pub fn main() {
    ...
    validate::eq(34, solve(sample), "day08 part02 sample");

    let input = include_str!("day08.input");
    validate::eq(1229, solve(input), "day08 part02 input");
}
